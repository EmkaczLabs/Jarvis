...
# Use Jarvis engine but keep the Glados type name for backwards compatibility
from jarvis.core.engine import Jarvis as Glados, JarvisConfig as GladosConfig
from jarvis.glados_ui.text_resources import aperture, help_text, login_text, recipe
...
class SplashScreen(Screen[None]):
    ...
    try:
        with open(Path("src/jarvis/glados_ui/images/splash.ansi"), encoding="utf-8") as f:
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        Any other key will start the Jarvis engine and then dismiss the splash screen.
...
class TypeInputScreen(ModalScreen[None]):
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if not text:
            app.notify("Empty input ignored", title="Jarvis", severity="warning")
            return

        if app.glados_engine_instance is None:
            app.notify("Jarvis engine not ready yet", title="Jarvis", severity="warning")
            return

        try:
            app.glados_engine_instance.submit_text(text)
            app.notify("Input sent", title="Jarvis")
        except Exception as e:
            logger.opt(exception=True).error(f"Failed to send typed input: {e}")
            app.notify("Failed to send input", title="Jarvis", severity="error")
        finally:
            self.dismiss()
...
class GladosUI(App[None]):
    ...
    try:
        with open(Path("src/jarvis/glados_ui/images/logo.ansi"), encoding="utf-8") as f:
