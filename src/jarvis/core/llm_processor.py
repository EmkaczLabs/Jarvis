...
class LanguageModelProcessor:
    ...
    def __init__(
        ...
        # State used to suppress tokens coming from inside internal thought tags
        self._suppress_think: bool = False
    ...
    def _process_chunk(self, line: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            if line.get("done_marker"):  # Handle [DONE] marker
                ...
                if not content:
                    return None
                return self._sanitize_chunk(str(content))
            # Handle Ollama format
            else:
    ...
    def _sanitize_chunk(self, content: str) -> str | None:
        """Sanitize a single chunk of text from the LLM stream.

        This method removes content between <think>...</think> (even if the opening
        and closing tags arrive in different chunks) and strips any remaining
        angle-bracket markup. Returns the cleaned content, or None if nothing
        should be emitted for this chunk.
        """
        # If we're inside a multi-chunk <think>...</think> span, drop until we see a closing tag
