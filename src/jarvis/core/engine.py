"""
Core engine module for the Jarvis voice assistant.

This module provides the main orchestration classes including the Jarvis assistant,
...
class JarvisConfig(BaseModel):
    """
    Configuration model for the Jarvis voice assistant.
    ...
    def from_yaml(cls, path: str | Path, key_to_config: tuple[str, ...] = ("Jarvis", "Glados")) -> "JarvisConfig":
        """
        Load a JarvisConfig instance from a YAML configuration file.

        Parameters:
            path: Path to the YAML configuration file
            key_to_config: Tuple of keys to navigate nested configuration. Accepts 'Jarvis' first then falls back to 'Glados'.

        Returns:
            JarvisConfig: Configuration object with validated settings
...
class Jarvis:
    ...
    This class manages the components of the Jarvis voice assistant, including speech recognition,
    ...
    def __init__(
        ...
        """
        Initialize the Jarvis voice assistant with configuration parameters.
    ...
    @classmethod
    def from_config(cls, config: JarvisConfig) -> "Jarvis":
        """
        Create a Jarvis instance from a JarvisConfig configuration object.

        Parameters:
            config (JarvisConfig): Configuration object containing Jarvis initialization parameters

        Returns:
            Jarvis: A new Jarvis instance configured with the provided settings
        """
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "Jarvis":
        """
        Create a Jarvis instance from a configuration file.

        Parameters:
            path (str): Path to the YAML configuration file containing Jarvis settings.

        Returns:
            Jarvis: A new Jarvis instance configured with settings from the specified YAML file.
        """
        return cls.from_config(JarvisConfig.from_yaml(path))
    ...
    def run(self) -> None:
        ...
        This method is the main entry point for running the Jarvis voice assistant.
        """
...
if __name__ == "__main__":
    # Prefer jarvis_config.yaml (rebranded) if present, otherwise fall back to the legacy glados_config.yaml
    from pathlib import Path

    config_candidates = ("jarvis_config.yaml", "glados_config.yaml")
    config_path = next((p for p in config_candidates if Path(p).exists()), "jarvis_config.yaml")
