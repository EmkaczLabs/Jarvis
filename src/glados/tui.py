...
class GladosUI(App[None]):
    ...
    def on_mount(self) -> None:
        ...
        self.notify("Loading AI engine...", title="Jarvis", timeout=6)
    ...
    def on_worker_state_changed(self, message: Worker.StateChanged) -> None:
        ...
        if message.state == WorkerState.SUCCESS:
            self.notify("AI Engine operational", title="Jarvis", timeout=2)
    ...
    @classmethod
    def run_app(cls, config_path: str | Path = "jarvis_config.yaml") -> None:
        """
        Run the TUI application connected to a configuration file.

        Parameters:
            config_path: Path to the configuration file to load (default: `jarvis_config.yaml`)
        """
        app = cls()
        app.run()
